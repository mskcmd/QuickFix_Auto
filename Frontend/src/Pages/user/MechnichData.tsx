...
import { FaMapMarkerAlt, FaTools, FaStar, FaTimes } from "react-icons/fa";
...
const MechanicCard: React.FC<{ mechanic: MechanicProfile }> = ({
  ...
  <article className="bg-white border border-gray-200 rounded-lg overflow-hidden transition-all duration-300 hover:shadow-lg">
    <img
      src={
        mechanic.profileImages[0]?.url || "https://via.placeholder.com/300x150"
      }
  ...
  return (
    ...
    <div className="bg-white rounded-lg shadow-2xl p-8 w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          type="submit"
          className="w-full bg-blue-600 text-white p-3 rounded-lg font-semibold hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-300"
        >
          Search
        </button>
      </form>
...
const MechanicListingPage: React.FC = () => {
  ...
  const filteredData = userSearchData.filter(
    (mechanic) => filter === "All" || mechanic.type === filter
  );
          ...
          <section className="">
            <div className="bg-white rounded-lg shadow-lg p-6">
              <div className="flex justify-center mb-6 space-x-4">
                {["All", "Shop", "Company", "Freelance"].map((option) => (
                  <button
                    key={option}
