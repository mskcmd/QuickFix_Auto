...
import { FaComments, FaUser, FaCog, FaHome, FaCogs, FaBlog, FaBars, FaTimes, FaSignInAlt, FaSignOutAlt, FaCommentAlt, FaExclamationTriangle, FaInfoCircle } from 'react-icons/fa';
...
function Header() {
  ...
  const NavButton: React.FC<NavButtonProps> = ({ icon: Icon, children, to, onClick }) => {
    const ButtonContent = (
      <>
  ...
  const DropdownItem: React.FC<NavLinkProps> = ({ to, icon: Icon, children }) => (
    <Link
      to={to}
    ...
    <header className="bg-black text-white py-3 px-4 shadow-lg">
      <div className="container mx-auto flex items-center justify-between">
        <div className="flex items-center space-x-1">
          <img src={Logo} alt="Logo" className="h-8 w-8 sm:h-10 sm:w-10 rounded-full" />
          ...
          <div className="relative">
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-black rounded-md shadow-lg py-1 z-10">
                {userData ? (
                  <>
                    <DropdownItem to="/logout" icon={FaSignOutAlt}>Logout</DropdownItem>
                    <DropdownItem to="/feedback" icon={FaCommentAlt}>Feedback</DropdownItem>
                    <DropdownItem to="/report" icon={FaExclamationTriangle}>Report</DropdownItem>
                  </>
                ) : (
                  <>
                    <DropdownItem to="/login" icon={FaSignInAlt}>Login</DropdownItem>
                    <DropdownItem to="/about" icon={FaInfoCircle}>About</DropdownItem>
                    <DropdownItem to="/blog" icon={FaBlog}>Blog</DropdownItem>
                  </>
                )}
              </div>
            )}
          </div>
          <button className="md:hidden text-xl hover:text-purple-400 transition-all duration-300 ml-2" onClick={toggleMenu}>
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden ${isMenuOpen ? 'block' : 'hidden'} bg-gray-800 text-white py-4 px-8 mt-3`}>
